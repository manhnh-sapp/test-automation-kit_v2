 ...
 * Các hàm xử lý chung cho việc tích hợp Jira, Confluence và Figma
 */
...
function normalizeEnvAliases() {
  if (!process.env.JIRA_BASE_URL && process.env.JIRA_URL) process.env.JIRA_BASE_URL = process.env.JIRA_URL;
  if (!process.env.JIRA_EMAIL && process.env.JIRA_USERNAME) process.env.JIRA_EMAIL = process.env.JIRA_USERNAME;
  if (!process.env.CONFLUENCE_USERNAME && process.env.JIRA_EMAIL) process.env.CONFLUENCE_USERNAME = process.env.JIRA_EMAIL;

  if (!process.env.CONFLUENCE_URL && process.env.JIRA_BASE_URL) {
    const jiraUrl = process.env.JIRA_BASE_URL.replace(/\/+$/, '');
    if (/\.atlassian\.net$/i.test(jiraUrl)) {
      process.env.CONFLUENCE_URL = `${jiraUrl}/wiki`;
    }
  }
}
...
function buildJiraHeaders() {
  ...
  const email = (process.env.JIRA_EMAIL || process.env.JIRA_USERNAME || '').trim();
  const patToken = (process.env.JIRA_PAT || '').trim();
...
/**
 * Resolve tên hiển thị / email -> Jira accountId (dùng cho assignee của Test/Test Plan/Test Execution).
 * Nếu truyền sẵn accountId (dạng "<realm>:<uuid>" hoặc hex dài) thì trả nguyên, không gọi API.
 * @param {string} query - display name, email, hoặc accountId
 * @returns {Promise<string|null>} accountId hoặc null nếu không tìm thấy
 */
async function resolveJiraAccountId(query) {
  const q = String(query || '').trim();
  if (!q) return null;
  // accountId Jira Cloud: "<realm>:<uuid>" hoặc chuỗi hex >=24 ký tự -> dùng thẳng
  if ((/:/.test(q) && /[0-9a-f-]{20,}/i.test(q)) || /^[a-f0-9]{24,}$/i.test(q)) return q;
  const base = (process.env.JIRA_BASE_URL || process.env.JIRA_URL || '').replace(/\/+$/, '');
  if (!base) return null;
  try {
    const res = await fetch(`${base}/rest/api/3/user/search?query=${encodeURIComponent(q)}`, { headers: buildJiraHeaders() });
    if (!res.ok) return null;
    const arr = await res.json();
    // ưu tiên khớp displayName/email chính xác; nếu không, lấy kết quả đầu tiên
    const exact = (arr || []).find((u) => String(u.displayName || '').trim() === q
